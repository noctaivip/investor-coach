...
export default {async fetch(req,env){const rid=requestId(req),url=new URL(req.url),started=Date.now();try{
 ...
 if(url.pathname==="/healthz")return out(req,env,{ok:true,service:"investor-coach-ai",version:"27.0.0"},200,rid);
 if(url.pathname==="/readyz")return out(req,env,{ok:!!env.OPENAI_API_KEY,openaiConfigured:!!env.OPENAI_API_KEY,version:"26.0.0"},env.OPENAI_API_KEY?200:503,rid);
