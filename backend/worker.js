...
Если режим sim:
- веди себя как опытный VC/investment committee interviewer, а не как учебный чат;
- держи в памяти всю доступную историю встречи: утверждения, цифры, определения и обещания пользователя;
- если пользователь назвал метрику или число без периода, сегмента, источника или контекста, следующий вопрос должен при необходимости проверить именно недостающую часть;
- если ответ уклончивый, сначала добейся прямого ответа, а не переключай тему;
- если ответ сильный, повышай сложность: проверяй причинность, устойчивость метрики, downside/risk или связь с экономикой;
- термин считай освоенным только если пользователь применяет его по смыслу; простое произнесение слова не является знанием;
- не повторяй уже закрытый вопрос другими словами без новой причины;
...
next_drill выбери по главному дефициту навыка. Связывай следующий drill с наблюдаемой ошибкой: если человек знает определение, но не умеет использовать термин в ответе инвестору — learning_gap=application или speech, а не meaning. Если одна и та же слабость повторяется в истории, усиливай её приоритет.
...
export default {async fetch(req,env){const rid=requestId(req),url=new URL(req.url),started=Date.now();try{
 ...
 if(url.pathname==="/healthz")return out(req,env,{ok:true,service:"investor-coach-ai",version:"28.0.0"},200,rid);
 if(url.pathname==="/readyz")return out(req,env,{ok:!!env.OPENAI_API_KEY,openaiConfigured:!!env.OPENAI_API_KEY,version:"28.0.0"},env.OPENAI_API_KEY?200:503,rid);
 if(url.pathname==="/api/transcribe"&&req.method==="POST"){if(!env.OPENAI_API_KEY)return out(req,env,{error:"ai_not_configured"},503,rid);const incoming=await req.formData(),file=incoming.get("file");if(!file||typeof file==="string")return out(req,env,{error:"audio_file_required"},400,rid);if(file.size>24*1024*1024)return out(req,env,{error:"audio_too_large"},413,rid);const fd=new FormData();fd.append("file",file,file.name||"answer.webm");fd.append("model",env.TRANSCRIBE_MODEL||"gpt-4o-transcribe");fd.append("language",incoming.get("language")||"ru");const context=String(incoming.get("context")||"").slice(0,5000);fd.append("prompt",`Точная транскрипция русской деловой речи для живого разговора с инвестором. Не исправляй смысл и не дублируй слова из-за пауз. Особенно точно сохраняй числа, проценты, валюты и термины: SAFE, cap table, valuation, dilution, runway, burn rate, CAC, LTV, MRR, ARR, NRR, GRR, churn, retention, traction, TAM, SAM, SOM, ICP, PMF, GTM, SaaS, unit economics, gross margin, EBITDA, pitch, term sheet, due diligence. Контекст встречи: ${context}`);const r=await fetch("https://api.openai.com/v1/audio/transcriptions",{method:"POST",headers:{Authorization:`Bearer ${env.OPENAI_API_KEY}`},body:fd}),raw=await r.text();if(!r.ok)return out(req,env,{error:"transcription_failed",detail:raw.slice(0,300)},502,rid);const parsed=JSON.parse(raw),text=String(parsed.text||"").replace(/\s+/g," ").trim();return out(req,env,{text},200,rid)}
